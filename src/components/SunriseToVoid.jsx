...
const SunriseToVoid = ({ scrollProgress, reverse = false }) => {
  ...
  const drawCurtainEffect = (
    ...
    const linePositions = isReverse
      ? baseLinePositions.map((pos) => 1 - pos)
      : baseLinePositions;
    ...
    if (effectiveProgress > fadeStartProgress) {
      fadeOpacity = Math.max(
        0,
        1 -
          (effectiveProgress - fadeStartProgress) /
    ...
    if (fadeOpacity > 0) {
      linePositions.forEach((linePos, index) => {
        ...
        const controlY =
          ...
          bendDirection *
            bendHeight *
            (1 - Math.abs(0.5 - (isReverse ? 1 - linePos : linePos)) * 2);
        ...
        if (effectiveProgress < 0.8) {
          const fillFadeOpacity =
            effectiveProgress < 0.6
              ? 1
              : Math.max(0, 1 - (effectiveProgress - 0.6) / 0.2);
      ...
      if (effectiveProgress > 0.2) {
        ...
        const sparkleFadeOpacity =
          effectiveProgress < 0.8
            ? 1
            : Math.max(0, 1 - (effectiveProgress - 0.8) / 0.15);
        ...
        for (let i = 0; i < sparkleCount; i++) {
          ...
          ctx.globalAlpha =
            Math.random() * 0.8 * sparkleFadeOpacity * fadeOpacity;
          ...
          // Add sparkle cross
          ctx.globalAlpha =
            Math.random() * 0.6 * sparkleFadeOpacity * fadeOpacity;
